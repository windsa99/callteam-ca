...
const failures = [];
const waveIds = new Set(Array.from({ length: 10 }, (_, index) => `CT-R${String(71 + index).padStart(3, "0")}`));
...
assert(wave.length === 10, `Expected 10 Wave Twelve scripts, found ${wave.length}.`);
for (const key of ["id", "slug", "title", "seoTitle", "metaDescription", "primaryKeyword"]) {
  assert(new Set(wave.map((script) => normalize(script[key]))).size === wave.length, `Wave Twelve ${key} values must be unique.`);
}
...
for (const script of wave) {
  assert(script.publishedDate === "2026-09-02", `${script.slug}: sandbox publication date is wrong.`);
  ...
  for (const other of wave) {
    ...
    assert(score < 0.12, `${script.slug} and ${other.slug}: Wave Twelve five-word similarity is ${score.toFixed(3)}.`);
  }
...
if (failures.length) {
  console.error(`Wave Twelve validation failed with ${failures.length} problem(s):`);
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log(`Wave Twelve validation passed for ${wave.length} scripts. Highest new-pair similarity: ${highestWavePair.score.toFixed(3)} (${highestWavePair.pair}). Highest legacy comparison: ${highestLegacyPair.score.toFixed(3)} (${highestLegacyPair.pair}).`);
